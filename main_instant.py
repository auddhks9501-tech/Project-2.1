...
# -*- coding: utf-8 -*-
"""
부호 층 복사 자동화  —  APT_PIT 층 뿌리기 v1.6-즉시실행
...
def parse_individual_floors(s):
    """
    개별 처리할 층 문자열을 파싱해 층 번호 집합(set[int])으로 변환.

    지원 형식 (콤마로 여러 개 나열 가능):
      - 단일 층      : '2', 'B3'
      - 범위(물결)   : '3~11', 'B4~B1'   (순서 무관, 3~11 = 11~3)
      - 지하/지상 혼합 범위: 'B2~3'  → 0층은 자동으로 건너뜀 (B2,B1,1,2,3)

    빈 문자열이면 빈 set 반환.
    형식이 잘못된 토큰은 원문을 그대로 보여주는 명확한 오류를 발생시킨다
    (업무용 자동화이므로, 애매하게 무시하지 않고 즉시 알려서 오입력을 막는다).
    """
    result = set()
...
def build_labels(start_str, end_str, individual_str, mark_end_e):
    ...
    개별 지정 층(단일 또는 '3~11' 범위)은 각각 단독 라벨로, 나머지 연속
    구간은 'A~B' 로 그룹핑, mark_end_e=True 이면 마지막 층에 'E' 접미사
    """
    ...
    individual_set = parse_individual_floors(individual_str)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("부호 층 복사 자동화  —  APT_PIT 층 뿌리기  v1.6-즉시실행")
    ...
    def _build(self):
        ...
        tk.Label(h, text="APT_PIT 층 뿌리기  v1.6-즉시실행", bg=PANEL, fg=SUB,
        ...
        tk.Label(row2, text="개별 처리할 층 (콤마 구분, 범위는 ~ 로 표기 가능, 예: 2, 5~9, B4~B1)",
