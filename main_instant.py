...
# -*- coding: utf-8 -*-
"""
부호 층 복사 자동화  —  APT_PIT 층 뿌리기 v1.4-즉시실행
...
def _type(text):
    s = str(text)
    if HAS_KB: keyboard.write(s, delay=0.015)
    elif HAS_PAG: pyautogui.typewrite(s, interval=0.02)
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("부호 층 복사 자동화  —  APT_PIT 층 뿌리기  v1.4-즉시실행")
    ...
    def _build(self):
        ...
        tk.Label(h, text="APT_PIT 층 뿌리기  v1.4-즉시실행", bg=PANEL, fg=SUB,
